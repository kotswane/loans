...
namespace StaffLoans.Controllers
{
    public class MyAccountController :BaseController 
    {
        ...
        public ActionResult Index()
        {
            ...
            try
            {
                ...
                using (var proxy = new LoanAPI.FINBONDAPIClient())
                {
                    ...
                    conReply = proxy.ConnectionCheck(0, 0, Convert.ToUInt32(ConfigurationManager.AppSettings["Origin"].ToString()));
                ...
                if (clientReply != null)
                {
                    ...
                    {
                        return RedirectToAction("Index", "Welcome", new { strMsg = "SE_" + clientReply.ReplyData.ReplyMessage + "" });
                    }
                ...
                if (conReply != null)
                {
                    if (conReply.LookupValues != null)
                    {
                        ...
                        if (mod.DocumentTypes.Exists(m => m.DocumentTypeId == "1"))
                        {
                            var docToRemove = mod.DocumentTypes.First(m => m.DocumentTypeId == "1");
                            mod.DocumentTypes.Remove(docToRemove);
                        }
                        #endregion
                        ...
                        mod.BankList  = (
                                               from r in conReply.BankDetails.ToList()
                                               select new BanksList()
                                               {
                                                   BankId  = r.BankName.ToString(),
                                                   BankDesc = r.BankName
                                               }
                                         ).ToList();
                        ...
                        mod.repaymentMethodList = (
                                ...
                                select new RepaymentMethodList()
                                {
                                    ...
                                    RepaymentMethodDesc  = (r.Description=="NAEDO"? "Debit Order" : "NAEDO")
                                }
                          ).ToList();
                        #endregion
                ...
                if (System.IO.File.Exists(HttpContext.Server.MapPath(viewContractPath)))
                {
                    ...
                    htmlCodeToConvert = htmlCodeToConvert.Substring(3).Replace("[FIRSTNAME]", "" + StaffLoans.Models.Common.LoggedInClientName + "").Replace("[MONTH]", "" + mfi.GetMonthName(DateTime.Today.Month) + "").Replace("[YEAR]", "" + DateTime.Today.Year + "");
                    htmlCodeToConvert = htmlCodeToConvert.Replace("[DAY]", "" + DateTime.Today.Day + "" + ((DateTime.Now.Day % 10 == 1 && DateTime.Now.Day != 11) ? "st": (DateTime.Now.Day % 10 == 2 && DateTime.Now.Day != 12) ? "nd": (DateTime.Now.Day % 10 == 3 && DateTime.Now.Day != 13) ? "rd": "th"));
                   // htmlCodeToConvert = htmlCodeToConvert.Replace("[SIGNEDAT]", "Rosebank");
                    //.Replace("[DAY]", "" + DateTime.Today.Day + "")
                    ...
                    htmlCodeToConvert = htmlCodeToConvert.Replace("[CONSUMERNAME]", "" + StaffLoans.Models.Common.LoggedInClientName + " " + "" + Common.LoggedInClientSurName + "").Replace("[DAY]", "" + DateTime.Today.Day + "").Replace("[MONTH]", "" + mfi.GetMonthName(DateTime.Today.Month) + "").Replace("[YEAR]", "" + DateTime.Today.Year + "");
                    ...
                    mod.salaryAndExpenses.Groceries = string.Empty;
                    mod.salaryAndExpenses.Transport = string.Empty;
                    mod.salaryAndExpenses.Housing = string.Empty;
                    mod.salaryAndExpenses.Clothing = string.Empty;
                    mod.salaryAndExpenses.Medical = string.Empty;
                    mod.salaryAndExpenses.WaterElectricty = string.Empty;
                    mod.salaryAndExpenses.ChildMaintainance = string.Empty;
                    mod.salaryAndExpenses.Education = string.Empty;
                    mod.salaryAndExpenses.BankCharges = string.Empty;
                    mod.salaryAndExpenses.Other = string.Empty;
        ...
        public ActionResult SaveEmployemntDetails(StaffLoans.Models.MyAccount modal, string sendOTP, string saveData)
        {
            ...
            try
            {
                ...
                using (var proxy = new LoanAPI.FINBONDAPIClient())
                {
                    ...
                    empData.ClientIDNumber = Common.LoggedInClientIDNumber;
        ...
        public ActionResult SaveBankDetails(StaffLoans.Models.MyAccount modal, string sendOTP, string saveData)
        {
            ...
            try
            {
                ...
                using (var proxy = new LoanAPI.FINBONDAPIClient())
                {
                    ...
                    bankData.BranchCode = "";
                    bankData.BankName = modal.bankDetails.Bank;
                    bankData.ClientIDNumber = Common.LoggedInClientIDNumber;
                    ...
                    if(insUpReply.ReplyData.ReplyCode ==0)
                    {
                        dataResult.Add("Success");
                    }
                    else
                    {
                        dataResult.Add(insUpReply.ReplyData.ReplyMessage );
                    }
            ...
            try {
                ...
                if (Common.getClientSessionData().SessionID != "")
                {
                    using (var proxy = new LoanAPI.FINBONDAPIClient())
                    {
                        ...
                        info.EmploymentType = Convert.ToUInt32(modal.employmentDetails.OccupationType);
        ...
        #region LoanAmount
        [HttpPost]
        public JsonResult retrieveLoanAmount(string loanAmount)
        {
            try
            {
                Session["LoanAmountReference"] = loanAmount;

                return Json("Saved");
            }
            catch (Exception ex)
            {
                errorLogger.Info("MyAccountController=>retrieveLoanAmount " + ex.Message + " " + ex.StackTrace);
            }
            return Json("Unable to Save");
        }
        #endregion

        [Filters.ValidateJsonAntiForgeryTokenAttribute]
        [HttpPost]
        public JsonResult CompleteLoanProcess(string strHTM,int iOfferID, string loanAmount)
        {
            string CultureName = System.Threading.Thread.CurrentThread.CurrentCulture.Name;
            CultureInfo ci = new CultureInfo(CultureName);
            if (ci.NumberFormat.NumberDecimalSeparator != ".")
            ...
            try
            {
                ...
                try
                {
                    var amt = Convert.ToDecimal(loanAmount.Substring(2));
                }
                catch(Exception e)
                {
                    return Json("Failed");
                ...
                if (Common.ClientLoginSessionId != "")
                {
                    using (var proxy = new LoanAPI.FINBONDAPIClient())
                    {
                        wLogger.Info("InsDocuments request");
                        ...
                        wLogger.Info("InsDocuments response");
                        if (docReply.ReplyCode == 0)
                        {
                            ...
                            if (loanComplettionReply.ReplyCode == 0)
                            {
                                ...
                                wLogger.Info("Loan Application Confirmation");
                                FinService.FinServiceClient client = new FinService.FinServiceClient();
                                client.SendSMS(Common.LoggedInClientContactNo, "FINBOND CREDIT: Your loan application for " + loanAmount + " has been successfully received. It is under review and subject to approval. Call 086 000 4249 for assistance", "FinLoan");
                                client.Close();
                                wLogger.Info("APPLICATION CONFIRMATION SENT TO:" + Common.LoggedInClientContactNo + " AT: " + DateTime.Now);
                                SendEmailToUser(Common.LoggedInClientEmailAddress, Common.LoggedInClientName, loanAmount);
                                return Json("Success");
                            }
                            else
                            {
        ...
        public void SendEmailToHR( string strEmailId, string strUserName)
        {
            ...
            sb.Append("<tr>");
            sb.Append("<td ><img  usemap=\"#mapFooLink\" src=\"cid:imgHEADER\" width=\"960\" height=\"80\" alt=\"Finbond\" /></td>");
            sb.Append("</tr>");
            //sb.Append("<tr><td style=\"background-color:#a7c1e4;height:4px\"></td></tr>");
            sb.Append("<tr><td style =\"font-family:arial;color:#4a4a4a\">");
            // sb.Append("<br />");
            sb.Append("<br />");
            sb.Append("<br/>");
            sb.Append("<br/>");
            ...
            sb.Append("<br/>");
            sb.Append("There is new loan request from Finbond Loans, Please click on below link to Approve/Reject the application");
            ...
            sb.Append("Finbond Loans Team");
            sb.Append("<br />");
            sb.Append("</td>");
            sb.Append("</tr>");
            sb.Append("<tr>");
            sb.Append("<td ><img  src=\"cid:imgFOOTER\" width=\"960\" height=\"124\" alt=\"FINBOND\" /></td>");
            sb.Append("</tr>");
            sb.Append("</table>");
            List<string> lst = new List<string>();
            lst.Add(strEmailId);
            Emails.SendEmail(sb, "Finbond Loan Request", lst);
            wLogger.Info("Finbond Loan Request:  " + lst + sb.ToString());
        }

        public void SendEmailToUser(string strEmailId, string strUserName,String strLoanAmt)
        {
            StringBuilder sb = new StringBuilder("");
            sb.Append("<table width=\"960px\" cellpadding=\"0\" cellspacing=\"0\">");
            sb.Append("<tr>");
            ...
            sb.Append("<br/>");
            sb.Append("You have successfully completed your loan application on Finbond Credit.");

            sb.Append("<br/>");
            sb.Append("<br/>");
            sb.Append("Your application is in review and subject to approval. Please allow 24 hours for response.");

            sb.Append("<br/>");
            sb.Append("<div style=\"display:inline-block\">Ensure all your profile information is correct. To update anything, please click &nbsp;</div>");
            sb.Append("<a style=\"display:inline-block\" href=\"https://www.finbondcredit.co.za/profile \"> [HERE] </a>");
            sb.Append("<br/>");
            sb.Append("<br/>");
            ...
            lst.Add(strEmailId);
            Emails.SendEmail(sb, "Finbond Loan Request", lst);
            wLogger.Info("Finbond Loan Request  Email to user:  " + lst + sb.ToString());
        }
