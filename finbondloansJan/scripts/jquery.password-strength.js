...
(function($) {
    ...
    $.fn.password_strength = function(options) {
        ...
        return this.each(function() {
            ...
            var container = $('<span id="spanEText"></span>')
            .css({  top: barTopPos - 6, left: barLeftPos + 15, 'font-size': '75%', display: 'none', width: password_settings.barWidth + 40,'margin-top':'5px' });

            //add the container next to textbox
            $(this).after(container);

            //bar border and indicator div
            var passIndi = $('<div id="PasswordStrengthBorder" style="margin-top:10px;"></div><div id="PasswordStrengthBar" class="BarIndicator"></div>')
            .css({  display: 'none' })
            .eq(0).css({ height: 4, top: barTopPos - 16, left: barLeftPos + 15, 'border-style': 'solid', 'border-width': 1, padding: 2 }).end()
            ...
            container.before("<div id='showhide' onclick='javascript:fnShowHidePasswordText()'  class='btnHidePassword ' ></div><div style='clear:both'></div><div id='divBreak' style='display:none'></div>");
            ...
            $(this).keyup(function() {
                ...
                if (passwordVal.length > 0) {
                    //alert(password_settings.useMultipleColors);
                    ...
                    $("[id$='spanEText']").css("display", "inline-block"); //hide
                    $("[id$='divBreak']").css("display", "block"); //hide
                    $("[id$='PasswordStrengthBorder']").css({ display: 'block', width: barWidth +4});
                    ...
                    if (password_settings.useMultipleColors == "1") {
                        ...
                        //33% to 66% is blue
                            ...
                            //debugger;
                            //alert(password_settings.barColor);
                            backColor ="green";
                        }
                    }

                    $("[id$='PasswordStrengthBar']").css({ display: 'block', width: strengthPercent +3, 'background-color': backColor,height: '7px', 'margin-top': '-8px' });
                    ...
                    $("[id$='spanEText']").css("display", "none"); //hide
                    $("[id$='divBreak']").css("display", "none"); //hide
